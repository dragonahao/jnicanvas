...
AccumulationSinglePhaseVaporAdimensionalized::AccumulationSinglePhaseVaporAdimensionalized(const AccumulationSinglePhaseVaporAdimensionalized &copy) : AccumulationFunction(copy.accumulationParams()){
    double mc = accumulationParams().component(0);
    double mw = accumulationParams().component(1);
    double Press = accumulationParams().component(2);
    double Tr = accumulationParams().component(3);
    double Tw = accumulationParams().component(4);
    double Cr = accumulationParams().component(5);
    double cw = accumulationParams().component(6);
    double T_typical = accumulationParams().component(7);
    double Rho_typical = accumulationParams().component(8);
    double U_typical = accumulationParams().component(9);
    double h_typical = accumulationParams().component(10);

    phi = accumulationParams().component(11);

    AccumulationSinglePhaseVaporAdimensionalized_Params *p = (AccumulationSinglePhaseVaporAdimensionalized_Params*)(&accumulationParams());

    thermo = copy.thermo;
}

AccumulationSinglePhaseVaporAdimensionalized::AccumulationSinglePhaseVaporAdimensionalized(const AccumulationSinglePhaseVaporAdimensionalized_Params &param, Thermodynamics *t) : AccumulationFunction(param){
    ...
    phi = param.component(11);

    AccumulationSinglePhaseVaporAdimensionalized_Params *p = (AccumulationSinglePhaseVaporAdimensionalized_Params*)(&param);
...
int AccumulationSinglePhaseVaporAdimensionalized::jet(const WaveState &w, JetMatrix &m, int degree) const{
    ...
    if (degree >= 0){
        ...
        double G0 = phi*rhosic;
        double G1 = phi*rhosiw;
        double G2 = Hr + phi*Hsi;
        ...
        if (degree >= 1){
            ...
            double dG0_dyw        = phi*drhosic_dyw;
            double dG0_dTheta     = phi*drhosic_dTheta;
            double dG0_dU         = 0.0;

            double dG1_dyw        = phi*drhosiw_dyw;
            double dG1_dTheta     = phi*drhosiw_dTheta;
            double dG1_dU         = 0.0;

            double dG2_dyw        = phi*dHsi_dyw;
            double dG2_dTheta     = dHr_dTheta + phi*dHsi_dTheta;
            double dG2_dU         = 0.0;
            ...
            if (degree >= 2){
                ...
                double d2G0dyw2      = phi*d2rhosic_dyw2;
                double d2G0dywdTheta = phi*d2rhosic_dywdTheta;
                double d2G0dywdU     = 0.0;

                double d2G0dThetadyw = d2G0dywdTheta;
                double d2G0dTheta2   = phi*d2rhosic_dTheta2;
                ...
                double d2G1dyw2      = phi*d2rhosiw_dyw2;
                double d2G1dywdTheta = phi*d2rhosiw_dywdTheta;
                double d2G1dywdU     = 0.0;

                double d2G1dThetadyw = d2G1dywdTheta;
                double d2G1dTheta2   = phi*d2rhosiw_dTheta2;
                ...
                double d2G2dyw2      = phi*d2Hsi_dyw2;
                double d2G2dywdTheta = phi*d2Hsi_dywdTheta;
                double d2G2dywdU     = 0.0;

                double d2G2dThetadyw = d2G2dywdTheta;
                double d2G2dTheta2   = phi*d2Hsi_dTheta2 + d2Hr_dTheta2;
